...
public class CarrentalController extends BaseController {
    ...
    protected IntentFilter getFilter() {
        ...
        filter.addAction(BroadcastAction.TRAVELINFO_GETCAR);
    ...
    protected void onHandle(Request request) {
        ...
        if (action.equals(Request.Action.GET_CAR)) {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(BroadcastAction.TRAVELINFO_GETCAR)) {
            CarRentalList list = (CarRentalList)intent.getSerializableExtra(CenterManager.CENTER_BROADCAST_RESULT);
            
            GetResultEvent<CarRentalList> event = new GetResultEvent<CarRentalList>();
            if (list != null) {
                event.result = list;
            }
