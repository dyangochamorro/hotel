...
public class BillqueryController extends BaseController {
    ...
    protected IntentFilter getFilter() {
        ...
        filter.addAction(BroadcastAction.ROOMSERVICE_BILLQUERY);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(BroadcastAction.ROOMSERVICE_BILLQUERY)) {
            BillList list = (BillList)intent.getSerializableExtra(CenterManager.CENTER_BROADCAST_RESULT);
            
            GetResultEvent<BillList> event = new GetResultEvent<BillList>();
            if (list != null) {
                event.result = list;
            }
